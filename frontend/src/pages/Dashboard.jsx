  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <p style={{ fontSize: '12px', color: 'var(--gray)', marginTop: '4px', lineHeight: 1.3, margin: 0 }}>{label}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        <StatCard
          num={loading ? '...' : data?.total_beneficiaires ?? 0}
          label={t('dashboard_total')}
          color="var(--green-dark)" bg="var(--green-light)"
          icon={<Icon name="people" size={22} color="#085041" />}
        />
        <StatCard
          num={loading ? '...' : data?.nb_agents ?? 0}
          label={t('dashboard_agents')}
          color="var(--blue-dark)" bg="var(--blue-light)"
          icon={<Icon name="collect" size={22} color="#1A4B7A" />}
        />
        <StatCard
          num={loading ? '...' : data?.zones?.length ?? 0}
          label={t('dashboard_zones')}
          color="var(--amber)" bg="var(--amber-light)"
          icon={<Icon name="location" size={22} color="#BA7517" />}
        />
        <StatCard
          num={loading ? '...' : data?.collectes_mois ?? 0}
          label={t('dashboard_collectes')}
          color="var(--red)" bg="var(--red-light)"
          icon={<Icon name="report" size={22} color="#A32D2D" />}
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        ...
        <div style={{ background: 'white', borderRadius: '10px', padding: '1.5rem', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.25rem' }}>
            ...
            <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#0D2E4E', margin: 0 }}>{t('dashboard_statut')}</h2>
          </div>
          {[
            ...
            <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: '600', color: item.ok ? 'var(--green-dark)' : 'var(--red)', background: item.ok ? 'var(--green-light)' : 'var(--red-light)', padding: '3px 10px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                <Icon name={item.ok ? 'check' : 'alert'} size={11} color={item.ok ? '#085041' : '#A32D2D'} />
                {item.status}
              </span>
            </div>
          ))}
