    ...
    <div>
      <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <h1 style={{ fontSize: '26px', fontWeight: '700', color: '#0D2E4E' }}>Tableau de bord</h1>
          <p style={{ color: 'var(--gray)', marginTop: '4px', fontSize: '14px' }}>Goma, Nord-Kivu · Données en temps réel</p>
        </div>
        <div style={{ background: 'var(--green-light)', border: '1px solid var(--green)', borderRadius: '8px', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '13px', color: 'var(--green-dark)', fontWeight: '600' }}>Système opérationnel</span>
        </div>
      </div>

      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        <StatCard num={loading ? '...' : data?.total_beneficiaires ?? 0} label="Bénéficiaires enregistrés" color="var(--green-dark)" bg="var(--green-light)" icon="👥" />
        <StatCard num={loading ? '...' : data?.nb_agents ?? 0} label="Agents actifs terrain" color="var(--blue-dark)" bg="var(--blue-light)" icon="🧑‍💼" />
        <StatCard num={loading ? '...' : data?.zones?.length ?? 0} label="Zones couvertes" color="var(--amber)" bg="var(--amber-light)" icon="📍" />
        <StatCard num={loading ? '...' : data?.collectes_mois ?? 0} label="Collectes ce mois" color="var(--red)" bg="var(--red-light)" icon="📋" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        <div style={{ background: 'white', borderRadius: '10px', padding: '1.5rem', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.25rem' }}>
            ...
            <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#0D2E4E' }}>Zones actives</h2>
          </div>
          {data?.zones?.length > 0 ? (
            ...
            <p style={{ color: 'var(--gray)', fontSize: '14px' }}>Aucune zone enregistrée.</p>
          )}
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '10px', padding: '1.5rem', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1.25rem' }}>
            ...
            <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#0D2E4E' }}>Statut du système</h2>
          </div>
          {[
